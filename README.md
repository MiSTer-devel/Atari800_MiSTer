...
### PokeyMax (Atari800)
One of the recent additions to the 800 core is the full integration of the PokeyMax implementation by Mark Watson. When disabled, the system behaves as if there are no extra Pokeys or sound devices, PokeyMax does not identify itself to the Atari, however, basic sound settings and mixing are still configurable. In general, the PokeyMax infrastrucutre is fully responsible for stereo (detection), channel selection and levels, mixing, etc. The top-level optiom from the main system additionally allows to define stereo mix (something that PokeyMax does not support). When enabled, all devices (4 Pokeys, 2 SIDs, 2 PSGs, Covox with a DMA sample engine) from the full PokeyMax config are available. Two feature details are different, there are only two output channels available (digital left and right, there are no analog channels, nor SPDIF provided by PokeyMax itself relying on the one provided by the main system), and there is no Flash support (but the persistent PokeyMax options are configurable from the OSD menu).
